    ...
    <>
      <Link to={`/produto/${id}/${sku}`}>
        <div className="w-75 h-100 flex flex-col justify-between select-none pointer-events-none">
          <div className="relative bg-white rounded-sm h-full flex justify-center items-center">
            {hasDiscount && (
            ...
            <img
              loading="lazy"
              className="object-fit overflow-hidden rounded-sm"
              src={image}
              alt={title}
            />
          </div>
          ...
          <div className="mt-2 select-text">
            <h6 className="text-light-gray font-bold text-xs mb-3">
              {category}
            </h6>
